...
One unified seed script — `app/prisma/seed.ts` — handles every subject from a registry at the top of the
file. There is no per-subject seed script; a subject is seedable as soon as its `flashcards.json` exists.
Run from `app/`:
